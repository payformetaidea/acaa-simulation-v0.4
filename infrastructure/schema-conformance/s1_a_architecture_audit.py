...
def main():
    ...
    results.append(check("evidence-on-failure", "if: always()" in S1V_WORKFLOW and "Upload S1-V evidence" in S1V_WORKFLOW, "evidence upload is unconditional"))
    results.append(check("deterministic-rerun", "Run validator twice" in S1V_WORKFLOW and "structured_identical" in S1V_WORKFLOW, "two-run comparison is explicit"))
    results.append(check("provenance-separation", "TARGET_COMMIT" in S1V_WORKFLOW and "validator_sha256" in S1V_WORKFLOW, "commit and file hashes are separately recorded"))
