...
class ValueFlowSim:
    ...
    def _detect_collusion(self):
        ...
        # Group agents based on shared CAU provenance (observable)
        for aid, agent in self.agents.items():
            if agent.isolated:
                continue
            ...
            if len(collusion_candidates) >= 2:
                ...
                edge_density = min(1.0, len(collusion_candidates) / 10.0)
                reciprocity = min(1.0, avg_overlap / 3.0)
                repeat_rate = min(1.0, total_overlap / 10.0)
    ...
    def _detect_spam(self):
        ...
        for agent in self.agents.values():
            ...
            # P0-4 FIX: Lowered threshold to 5 and adjusted condition
