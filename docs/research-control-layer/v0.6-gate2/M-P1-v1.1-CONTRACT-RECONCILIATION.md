...
```text
manifest
└── CAU records                 = implementation-defined extraction
...
Therefore M-P1-v1.0 cannot be treated as a valid consumer contract for this O2 artifact without changing either the artifact or the metric specification. Both actions would violate the current evidence boundary.

## 2. Canonical contract for the v1.1 draft

The v1.1 draft therefore adopts the already-frozen O2 boundary rather than introducing a translation layer.
...
The distinction from M-P1-v1.0 is retained:
...
A consumer must fail closed on a structurally unusable input. It must never turn a missing record container into an empty valid record set.

## 4. Provenance boundary

This document does **not** claim that the previously reported Python and JavaScript implementations are the v1.1 implementations. Their exact source provenance remains unresolved.

Accordingly:

- no implementation source is silently reclassified as v1.1;
- no external runtime behavior is promoted to specification evidence;
- no adapter is introduced;
- no existing raw artifact is modified;
...
parse artifact
    ...
    +-- canonicalize only where explicitly specified
