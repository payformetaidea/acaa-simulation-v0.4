...
`CV_between` is reported as a descriptive estimand. `CV_between > 0.10` is a diagnostic threshold for material seed-associated variability; it is **not itself a falsification condition for H-004**.
...
σ²_B,reported = max(0, (MS_B − MS_W) / r)
σ²_W = MS_W
T = σ²_B,reported / σ²_W
  = max(0, F − 1) / 3
  where F = MS_B / MS_W and r = 3
...
The null distribution is generated by permuting the 33 observed outcome values into 11 balanced groups of 3, preserving group size. For each permutation, the variance-component estimator is recomputed with the same non-negativity truncation and `T_perm` is calculated.

Decision for the hypothesis test:

- `p < 0.05` → evidence supports `σ²_B > 0`; H-004 is **DEMONSTRATED**, subject to all other Gate conditions.
- `p ≥ 0.05` → H-004 is **NOT DEMONSTRATED**; this does not establish `σ²_B = 0` and is never treated as falsification.

H-004 is direction-neutral with respect to the direction of seed effects. No signed-effect falsification rule is active in the current Gate-2 design.
...
- `H-004 = FALSIFIED` → no active Gate-2 falsification rule exists for H-004 under the present variance-component formulation.
