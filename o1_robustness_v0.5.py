...
def base_config_identity(data: Mapping[str, Any]) -> str:
    ...
    # v0.4 exposes config_hash as its seed-independent BaseConfig fingerprint.
...
def effective_config_identity(data: Mapping[str, Any]) -> str:
    ...
    # Prefer an explicit fingerprint if the engine emits one; otherwise hash
    # the canonical JSON representation exactly as exported.
...
def build_evidence(records: Sequence[Mapping[str, Any]], output_dir: Path) -> Dict[str, Any]:
    ...
    for field, _label in metric_names.items():
